...
pub async fn fetch_timetable_by_synergetic_id(
    synergetic_id: i32,
    user_id: i64,
    timetable_id: i32,
    pool: &Pool<Postgres>,
    ...
    let timetable_api_url = format!(
        "https://my.mgs.vic.edu.au/mg/api/timetable_week/{}/{}/S/student.json",
        synergetic_id,
        chrono::offset::Local::now().format("%Y-%m-%d")
    );
    let (simple_saml_session_id, simple_saml_auth_token_cookie, ssess_cookie) =
        login(username.as_str(), password.as_str()).await?;
    let cookie = format!(
        "{}; {}; {}",
        simple_saml_session_id, simple_saml_auth_token_cookie, ssess_cookie
    );
    ...
    let json: serde_json::Value =
        serde_json::from_str(&body.as_str()).expect("JSON was not formatted properly");
    let mut d_number = 1;
    ...
    for i in 1..=7 {
        ...
        let mut p_number = 1;
        for j in 2..=9 {
            // No. 2,3,5,6,8,9 for periods 1,2,3,4,5,6 (4 & 7 are Lunch and Recess)
            if j != 4 && j != 7 {
    ...
    println!(
        "Pushed classes to database for user {}: {}ms",
        user_id,
...
// Update
pub async fn update_timetable_by_user_id(
    user_id: i64,
    fetched_date: NaiveDate,
    pool: &Pool<Postgres>,
) -> Result<()> {
...
pub async fn delete_timetable_by_user_id(user_id: i64, pool: &Pool<Postgres>) -> Result<()> {
    ...
    let timetable_id = sqlx::query!(r#"SELECT id FROM timetables WHERE user_id = $1"#, user_id)
        .fetch_one(pool)
        .await?
        .id;
    ...
    sqlx::query!("DELETE FROM timetables WHERE user_id = $1", user_id)
        .execute(pool)
        .await?;
...
pub async fn get_timetable_id_by_user_id_if_it_exists(
    user_id: i64,
    pool: &Pool<Postgres>,
) -> Result<i32> {
    let records = sqlx::query!("SELECT id FROM timetables WHERE user_id = $1", user_id)
        .fetch_all(pool)
        .await?; // Should only be returning one record
