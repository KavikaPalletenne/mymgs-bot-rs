...
use crate::timetable::{
    get_timetable_by_user_id, get_timetable_id_by_user_id_if_it_exists, initialise_timetable,
};
...
#[group]
#[commands(help, start, tt, today, tomorrow, tmr, dates, compete, play)]
struct General;
...
async fn start(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
    ...
    if !range.contains(&synergetic_id) {
        msg.reply(ctx, "Synergetic ID is either too short or too long")
            .await?;
        pool.close().await;
        return Ok(());
    }
    ...
    if get_timetable_id_by_user_id_if_it_exists(user_id, &pool).await? != 0
        && get_timetable_by_user_id(user_id, &pool)
            .await?
            .fetched_date
    ...
    create_user_by_id(
        user_id,
        synergetic_id,
        "kbpalletenne@student.mgs.vic.edu.au",
        "12062004",
        &pool,
    )
    .await?;
    ...
    match timetable_response {
        ...
        _ => {
            msg.reply(
                ctx,
                "No such timetable exists. Ensure your Synergetic ID is correct.",
            )
            .await?;
            delete_user_by_id(user_id, &pool).await?;
        }
    ...
    //msg.reply(ctx,reply_msg).await?;
    println!(
        "Fetched timetable {} and created user: {}ms",
        synergetic_id,
...
async fn tt(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
    ...
    let error_message = format!(
        "Command error - ``{}`` is not a recognised command",
        functions[0]
    );
    match functions[0] {
        "today" => {
                ...
                || day_number == 0
            {
                msg.reply(
                    ctx,
                    "Tomorrow is a holiday so you don't have to worry about what classes you have!",
                )
                .await?;
                pool.close().await;
                return Ok(());
            ...
            match timetable_id {
                0 => {
                    msg.reply(ctx, "Your timetable is not saved with the bot, type ``$help`` for instructions.").await?;
                    pool.close().await;
                    return Ok(());
                }
                _ => {}
            ...
            let classes =
                get_all_classes_for_day_by_timetable_id(timetable_id, day_number, &pool).await?;
        ...
        "tomorrow" => {
            let today_date = chrono::offset::Local::now().naive_local().date();
            let tomorrow_date =
                NaiveDate::from_ymd(today_date.year(), today_date.month(), today_date.day() + 1); // TODO: If tomorrow is a holiday, get the next school day's timetable instead
            ...
            let classes =
                get_all_classes_for_day_by_timetable_id(timetable_id, day_number, &pool).await?;
            let mut message = format!(
                "{}, here is your timetable for tomorrow:\n```",
                msg.author.mention()
            );
            for i in 0..classes.len() {
                message.push_str(classes[i].name.as_str());
...
async fn today(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
    ...
    match timetable_id {
        0 => {
            msg.reply(
                ctx,
                "Your timetable is not saved with the bot, type ``$help`` for instructions.",
            )
            .await?;
            pool.close().await;
            return Ok(());
