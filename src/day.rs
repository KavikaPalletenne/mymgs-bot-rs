...
pub async fn create_day(date: NaiveDate, day_number: i16, pool: &Pool<Postgres>) -> Result<()> {
    let records = sqlx::query!("SELECT day_number FROM days WHERE date = $1", date)
        .fetch_all(pool)
        .await?; // Should not be returning any record
...
pub async fn delete_day_by_date(date: NaiveDate, pool: &Pool<Postgres>) -> Result<()> {
    sqlx::query!("DELETE FROM days WHERE date = $1", date)
        .execute(pool)
        .await?;
...
pub async fn get_day_numbers(iterations: i16, pool: &Pool<Postgres>) -> Result<()> {
    ...
    let timetable_api_url = format!(
        "https://my.mgs.vic.edu.au/mg/api/timetable_week/102760/{}/S/student.json",
