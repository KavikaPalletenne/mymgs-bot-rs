...
//Read
pub async fn get_all_classes_by_timetable_id(
    timetable_id: i32,
    pool: &Pool<Postgres>,
) -> Result<Vec<Class>> {
...
// Delete
pub async fn delete_all_classes_in_timetable(
    timetable_id: i32,
    pool: &Pool<Postgres>,
) -> Result<()> {
    sqlx::query!("DELETE FROM classes WHERE timetable_id = $1", timetable_id)
        .execute(pool)
        .await?;
...
pub async fn get_all_classes_for_day_by_timetable_id(
    timetable_id: i32,
    day_number: i16,
    pool: &Pool<Postgres>,
) -> Result<Vec<Class>> {
